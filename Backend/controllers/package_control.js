...
exports.getOnePackage = (req, res) => {
  ...
  Package.find({
    ...
    .populate({
      path: "day_meal.meal_1",
      model: "Menu"
    })
    .populate({
      path: "day_meal.meal_2",
      model: "Menu"
    })
    .exec((err, packages) => {
      if (err) {
  ...
  Package.findOne(
    ...
    (err, package) => {
      if (package) {
        ...
        for (var i = 0; i < 0; i++) {
          if (
            String(package.day_meal[i].meal_1) !=
            String(newPackage.day_meal[i].meal_1)
          ) {
            makeNewPackage = !makeNewPackage;
            console.log(
              package.day_meal[i].meal_1,
              newPackage.day_meal[i].meal_1
            );
            break;
          }
          if (
            String(package.day_meal[i].meal_2) !=
            String(newPackage.day_meal[i].meal_2)
          ) {
            makeNewPackage = !makeNewPackage;
            console.log(
              package.day_meal[i].meal_2,
              newPackage.day_meal[i].meal_2
            );
            break;
          }
        }
